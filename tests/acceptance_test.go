...
func TestWebhooksSemverUpdate(t *testing.T) {
	...
	t.Run("UpdateThroughDockerHubWebhook", func(t *testing.T) {
		...
		if err != nil {
			t.Errorf("failed to make a webhook request to quilla: %s", err)
		}

		if resp.StatusCode != 200 {
			t.Errorf("unexpected webhook response from quilla: %d", resp.StatusCode)
		}
...
// Test to ensure quilla doesn't try to be tolerant and parse integers as semver versions, for example
// 45000 shouldn't become 45000.0.0 version (https://github.com/quilla-hq/quilla/issues/296)
		...
		dep := &apps_v1.Deployment{
			...
			meta_v1.ObjectMeta{
				...
				Labels: map[string]string{
					...
					types.QuillaMinimumApprovalsLabel: "1",
					types.QuillaApprovalDeadlineLabel: "5",
				},
				Annotations: map[string]string{},
			},
			apps_v1.DeploymentSpec{
